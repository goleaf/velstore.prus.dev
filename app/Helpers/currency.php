...
if (! function_exists('activeCurrency')) {
    ...
    {
        $sessionCode = session('currency', getWebConfig('default_currency', 'USD'));

        $currency = Currency::where('code', $sessionCode)->first();
        if (! $currency) {
            $currency = Currency::where('code', 'USD')->first();
        }

        if ($currency) {
            Cache::forever('active_currency_'.$sessionCode, $currency);
        }
