            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary"
                            data-bs-dismiss="modal">{{ __('cms.products.cancel') }}</button>
                    <button type="button" class="btn btn-danger"
                            id="confirmDeleteProduct">{{ __('cms.products.delete') }}</button>
                </div>
            </div>
        </div>
    </div>
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            ...
                            <span class="border border-edit dt-trash rounded-3 d-inline-block">
                                <button type="button" class="btn btn-link p-0" data-url="/vendor/products/${row.id}/edit">
        ...
        function deleteProduct(id) {
            ...
            $('#confirmDeleteProduct').off('click').on('click', function() {
                if (productToDeleteId !== null) {
                    $.ajax({
                        url: '{{ route('vendor.products.destroy', ':id') }}'.replace(':id',
                            productToDeleteId),
                        method: 'DELETE',
                        data: {
                            _token: "{{ csrf_token() }}",
