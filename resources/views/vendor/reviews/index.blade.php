            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary"
                            data-bs-dismiss="modal">{{ __('cms.product_reviews.cancel') }}</button>
                    <button type="button" class="btn btn-danger"
                            id="confirmDeleteReview">{{ __('cms.product_reviews.delete') }}</button>
                </div>
            </div>
        </div>
    </div>
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            ...
                            <span class="border border-info rounded-3 d-inline-block">
                                <button type="button" class="px-2 btn btn-link p-0" data-url="/vendor/reviews/${row.id}">
        ...
        function deleteReview(id) {
            ...
            $('#confirmDeleteReview').off('click').on('click', function() {
                if (reviewToDeleteId !== null) {
                    $.ajax({
                        url: '{{ route('vendor.reviews.destroy', ':id') }}'.replace(':id',
                            reviewToDeleteId),
                        method: 'DELETE',
                        data: {
                            _token: "{{ csrf_token() }}",
