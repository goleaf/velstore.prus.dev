            ...
            <form action="{{ route('admin.banners.update', $banner->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select name="type" class="form-control" required>
                        <option value="promotion" {{ $banner->type == 'promotion' ? 'selected' : '' }}>
                            {{ __('cms.banners.promotion') }}</option>
                        <option value="sale" {{ $banner->type == 'sale' ? 'selected' : '' }}>{{ __('cms.banners.sale') }}
                        </option>
                        <option value="seasonal" {{ $banner->type == 'seasonal' ? 'selected' : '' }}>
                            {{ __('cms.banners.seasonal') }}</option>
                        <option value="featured" {{ $banner->type == 'featured' ? 'selected' : '' }}>
                            {{ __('cms.banners.featured') }}</option>
                        <option value="announcement" {{ $banner->type == 'announcement' ? 'selected' : '' }}>
                            {{ __('cms.banners.announcement') }}</option>
                    </select>
                </div>
                        ...
                        <div class="tab-content mt-3" id="languageTabContent">
                            @foreach ($languages as $index => $language)
                                ...
                                <div class="tab-pane fade show {{ $loop->first ? 'active' : '' }}"
                                    ...
                                    <div class="form-group">
                                        <label
                                               for="languages[{{ $index }}][title]">{{ __('cms.banners.title') }}</label>
                                    ...
                                    <div class="form-group">
                                        <label
                                               for="languages[{{ $index }}][description]">{{ __('cms.banners.description') }}</label>
                                    ...
                                    <label class="form-label mt-2">{{ __('cms.banners.image') }}
                                        ({{ $language->code }})</label>
                                    ...
                                    <input type="hidden" name="languages[{{ $index }}][language_code]"
                                           value="{{ $language->code }}">
                ...
                <button type="button" class="btn btn-secondary mt-3"
                        data-url="{{ route('admin.banners.index') }}">{{ __('cms.banners.button_back') }}</button>
            </form>
        </div>
    </div>
    <script>
