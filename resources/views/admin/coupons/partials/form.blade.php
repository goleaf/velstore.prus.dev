@php
    ...
    $formAction = $formAction ?? ($action ?? route('admin.coupons.store'));
    ...
    $formatDateTime = static function ($value, $timezone) {
        if (blank($value)) {
            return '';
        }

        try {
            return Carbon::parse($value)->timezone($timezone)->format('Y-m-d\\TH:i');
...
<x-admin.card :title="__('cms.coupons.form_title')" class="mt-6">
    <p class="text-sm text-gray-500 mb-6">{{ __('cms.coupons.form_description') }}</p>
    ...
    >
        ...
        <div class="space-y-2">
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                <input
                    ...
                    required
                >
                <span class="inline-flex h-10 items-center rounded-md border border-gray-200 bg-gray-50 px-3 text-sm font-medium text-gray-600"
                      x-text="type === 'percentage' ? '%' : @js(__('cms.coupons.discount_fixed_suffix'))"></span>
            </div>
        ...
        <div class="space-y-4 border-t border-gray-200 pt-6">
            <div>
                ...
                <p class="text-xs text-gray-500 mt-1">{{ __('cms.coupons.expiry_section_description') }}</p>
            </div>
