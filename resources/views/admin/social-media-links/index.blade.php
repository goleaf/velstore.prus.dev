...
@section('content')
    <div class="card mt-4">
        ...
        <div class="card-body">
            ...
            <button type="button" class="btn btn-success float-end mb-3"
                    data-url="{{ route('admin.social-media-links.create') }}">{{ __('cms.social_media_links.add_new') }}</button>
    ...
    <div class="modal fade" id="deleteSocialMediaLinkModal" tabindex="-1" aria-labelledby="deleteSocialMediaLinkModalLabel"
         aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="deleteSocialMediaLinkModalLabel">
                        {{ __('cms.social_media_links.massage_confirm') }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body"> {{ __('cms.social_media_links.confirm_delete') }}</div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary"
                            data-bs-dismiss="modal">{{ __('cms.social_media_links.massage_cancel') }}</button>
                    <button type="button" class="btn btn-danger"
                            id="confirmDeleteSocialMediaLink">{{ __('cms.social_media_links.massage_delete') }}</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        $('#confirmDeleteSocialMediaLink').off('click').on('click', function() {
            if (socialMediaLinkToDeleteId !== null) {
                $.ajax({
                    url: '{{ route('admin.social-media-links.destroy', ':id') }}'.replace(':id',
                        socialMediaLinkToDeleteId),
                    method: 'DELETE',
                    data: {
                        _token: "{{ csrf_token() }}",
                    },
